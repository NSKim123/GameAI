...
public:
	NeuralLayer(int layer_level = INPUT_LAYER, int inNodeSize = 1, int outNodeSize = 1, int active = NONE_ACTIVE)
	{
		...
		{
			m_W[i] = (((float)(rand() % 9000) - 4500.0f) / 4500.0f);//소수점이 다양해진다
			m_grad_w[i] = 0.0;
		}
	...
	void Backward(float* igrad_y)
	{
		...
		memset(m_grad_w, 0, sizeof(float)*(m_outNodeSize * m_inNodeSize));
