...
float correct_data1[INPUTDATA];
float correct_data2[INPUTDATA];
...
void Init_dataSet()
{
	for (int i = 0; i < INPUTDATA; i++)
	{
		...
		correct_data1[i] = cos(input_data[i]);
		correct_data2[i] = input_data[i] * 2.0;
	}
}
...
void DrawAxis(Mat& src)
{
	...
	for (int i = 0; i < (INPUTDATA - 1); i++)
	{
		...
		line(src, Point((input_data[i] / (PI * 2.0)) * 500, (correct_data1[i] * 250) + 250), Point((input_data[i + 1] / (PI * 2.0)) * 500, (correct_data1[i + 1] * 250) + 250), Scalar(0, 0, 255), 1);
	}
	...
	while (1)
	{
		...
		for (int i = 0; i < INPUTDATA; i++)
		{
			...
			if (frame % interval == 0)
			{
				...
				circle(src, Point((x[0] / (PI * 2.0)) * 500, (prey[1] * 250) + 250), 3, Scalar(0, 0, 255), -1);
				circle(src, Point((x[0] / (PI * 2.0)) * 500, prey[2] * 20), 3, Scalar(255, 0, 255), -1);
			}
		}
